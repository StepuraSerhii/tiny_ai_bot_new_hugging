...
def _generate_humor_angle(scene: str, theme_hint: str = "") -> str:
    """AI сам вигадує унікальний кут подачі жарту для конкретного фото і теми.
    Щоразу абсолютно новий — не з готового списку."""
    context = f"На фото: {scene}." if scene else ""
    theme_ctx = f' Тема: "{theme_hint}".' if theme_hint else ""
    try:
        r = groq_client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": (
                    f"{context}{theme_ctx}\n\n"
                    "Придумай ОДИН оригінальний, несподіваний кут подачі мем-жарту для цього фото.\n"
                    "Це має бути конкретна інструкція для іншого AI — з якої точки зору написати підпис.\n"
                    "Наприклад: «Уяви що це довідка від лікаря. Який діагноз?» або "
                    "«Внутрішній монолог об'єкта після третьої кави» або "
                    "«Це субтитр у документалці про людей які роблять вигляд що все добре».\n\n"
                    "ВИМОГИ:\n"
                    "- 1 речення, максимум 20 слів\n"
                    "- конкретний і образний, не абстрактний\n"
                    "- прив'язаний до того що на фото і теми\n"
                    "- кожного разу НОВИЙ, не шаблонний\n"
                    "- без пояснень, тільки сам кут\n"
                    "Відповідай одним реченням, без лапок і без пояснень."
                )
            }],
            max_tokens=60,
            temperature=1.5,
        )
        angle = r.choices[0].message.content.strip().strip('"').strip("'")
        logging.info(f"Generated humor angle: {angle!r}")
        return angle
    except Exception as e:
        logging.error(f"_generate_humor_angle: {e}")
        # Мінімальний fallback якщо щось пішло не так
        fallbacks = [
            "Внутрішній монолог людини на фото о 2 ночі.",
            "Це підпис під фото у груповому чаті сім'ї.",
            "Заголовок новини на ukr.net про цей момент.",
            "Що написав би ЖЕК у офіційному листі про цю ситуацію.",
        ]
...
def generate_meme_text(image_bytes: bytes, theme_hint: str = "", scene: str | None = None) -> tuple[str, str]:
    ...
    # AI сам придумує унікальний кут подачі для цього конкретного фото і теми
    chosen_angle = _generate_humor_angle(scene, theme_hint)
            ...
            messages=[{
                ...
                "content": [
                    ...
                    {"type": "text", "text": (
                        ...
                        f"{scene_line}"
                        f"{theme_line}"
                        f"\nКУТ ПОДАЧІ (важливо!): {chosen_angle}\n"
                        "\n"
                        ...
                        "- пояснювати очевидне з фото\n"
                        "- безпечний нейтральний гумор «для всієї родини»\n"
                        "- шаблонні фрази служби підтримки («нас рекомендували», «ваш запит»)\n"
                        "\n"
                        ...
                        "---\n"
                        "ВЕРХ: HR надіслав запит на зворотній зв'язок\n"
                        "НИЗ: Зворотній зв'язок: пиздець\n"
                        "\n"
